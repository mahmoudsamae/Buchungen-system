...
import { mergeTeacherSettingsRow } from "@/lib/teacher/teacher-settings-defaults";
...
export async function validateTeacherBookingPolicy(supabase, args) {
  const { business, customerUserId, bookingDateYmd, startHHMM, primaryInstructorUserId, membership, excludeBookingId } = args;
  ...
  if (error && error.code !== "42P01") {
    return { ok: false, message: "Could not load instructor policy." };
  }

  const s = mergeTeacherSettingsRow(raw || null);
