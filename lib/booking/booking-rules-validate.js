...
export async function validateBookingBusinessRules(supabase, opts) {
  ...
  const dailyLimitEnabled = business.max_bookings_per_day_enabled !== false;
  const dailyLimitCap = Math.max(1, Number(business.max_bookings_per_day) || 1);
  const weeklyLimitEnabled = business.max_bookings_per_week_enabled !== false;
  const weeklyLimitCap = Math.max(1, Number(business.max_bookings_per_week) || 2);
  ...
  if (business.booking_release_window_enabled) {
    const from = String(business.booking_allowed_from || "").slice(0, 5);
    const until = String(business.booking_allowed_until || "").slice(0, 5);
    const nowHm = String(zonedNowParts(tz).time || "").slice(0, 5);
    if (from && until && nowHm) {
      // Inclusive bounds: first minute and last minute are both allowed.
      if (nowHm < from || nowHm > until) {
        return {
          ok: false,
          code: "BOOKING_WINDOW_CLOSED",
          message: `Booking is currently available only between ${from} and ${until} (${tz}).`
        };
      }
    }
  }
  ...
  if (business.prevent_overlapping_bookings_enabled && end) {
    const { rows, err } = await listBookingsForDate(
      supabase,
      business.id,
      customerUserId,
      dateStr,
    ...
    if (err) return { ok: false, message: "Could not verify overlap rules." };
    const as = timeToMinutes(start);
    const ae = timeToMinutes(end);
    const hasOverlap = (rows || []).some((r) => {
      const rs = timeToMinutes(String(r.start_time || "").slice(0, 5));
      const re = timeToMinutes(String(r.end_time || "").slice(0, 5));
      if ([as, ae, rs, re].some((v) => v == null)) return false;
      return timesOverlapHalfOpenMinutes(as, ae, rs, re);
    });
    if (hasOverlap) {
      return { ok: false, code: "OVERLAPPING_BOOKING", message: "You already have a booking at that time." };
    }
  }
  ...
  if (dailyLimitEnabled) {
    ...
    if (n >= dailyLimitCap) {
      return {
        ...
        message: "You have reached the maximum number of bookings for this day."
      };
    }
  }

  if (weeklyLimitEnabled) {
    const weekAnchor = zonedStartToUtcDate(dateStr, "12:00", tz);
    const { start: w0, end: w1 } = weekYmdRangeInTimeZone(tz, weekAnchor);
    ...
    if (n >= weeklyLimitCap) {
      return {
        ...
        message: `You have reached the weekly limit of ${weeklyLimitCap} active appointment(s) for this business.`
      };
    }
  }
...
async function listBookingsForDate(supabase, businessId, customerUserId, dateYmd, excludeBookingId) {
  let q = supabase
    .from("bookings")
...
async function cancellationCount(supabase, businessId, customerUserId) {
  return supabase
    ...
    .eq("business_id", businessId)
    .eq("customer_user_id", customerUserId)
    .in("status", ["cancelled_by_user", "cancelled_by_manager"]);
}
