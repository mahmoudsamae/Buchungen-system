 ...
 */
export async function resolveTeacherWindowsForDate({ supabase, businessId, teacherUserId, dateStr, categoryId }) {
  ...
  let ovsQuery = supabase
    .from("teacher_availability_overrides")
    .select("id, is_closed, start_time, end_time, is_active, category_id")
    .eq("business_id", businessId)
    .eq("staff_user_id", teacherUserId)
    .eq("override_date", d)
    .eq("is_active", true);
  if (!hasNoCategoryFilter) {
    if (customerCategoryId) {
      ovsQuery = ovsQuery.or(`category_id.is.null,category_id.eq.${customerCategoryId}`);
    } else {
      ovsQuery = ovsQuery.is("category_id", null);
    }
  }
  const { data: ovs, error: oe } = await ovsQuery;
  ...
  let rulesQuery = supabase
    .from("teacher_availability_rules")
    .select("start_time, end_time, is_active, valid_from, valid_until, category_id")
    .eq("business_id", businessId)
    .eq("staff_user_id", teacherUserId)
    .eq("weekday", weekday)
    .eq("is_active", true);
