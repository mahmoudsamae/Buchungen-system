...
/** Next Monday-start week [start, end] as YYYY-MM-DD in `timeZone`. */
export function nextWeekYmdRangeInTimeZone(timeZone, instant = new Date()) {
  const z = toZonedTime(instant, timeZone);
  const next = addWeeks(z, 1);
  const ws = startOfWeek(next, { weekStartsOn: 1 });
  const we = endOfWeek(next, { weekStartsOn: 1 });
  return { start: format(ws, "yyyy-MM-dd"), end: format(we, "yyyy-MM-dd") };
}
...
/** Current local parts in a timezone. */
export function zonedNowParts(timeZone, instant = new Date()) {
  const z = toZonedTime(instant, timeZone);
  return {
