...
export async function getSchoolTeacherDetail(businessId, teacherUserId) {
  ...
  const { data: svcCatalog } = await admin
    .from("services")
    .select("id, name, duration_minutes, is_active, category_id")
    .eq("business_id", businessId)
    .order("name");

  const categoryIds = [...new Set((svcCatalog || []).map((s) => String(s.category_id || "")).filter(Boolean))];
  const { data: catRows } = categoryIds.length
    ? await admin
        ...
        .in("id", categoryIds)
    : { data: [] };
  const catNameById = Object.fromEntries((catRows || []).map((c) => [String(c.id), String(c.name || "")]));
  ...
  const servicesCatalog = (svcCatalog || []).map((s) => ({
    ...
    category_name: s.category_id ? catNameById[String(s.category_id)] || "" : ""
  }));
