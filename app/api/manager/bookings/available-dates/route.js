...
export async function GET(request) {
  ...
  const fromDate = normalizeDate(request.nextUrl.searchParams.get("fromDate"));
  const horizonDaysRaw = Number(request.nextUrl.searchParams.get("horizonDays") || 30);
  const horizonDays = Number.isFinite(horizonDaysRaw) ? Math.max(1, Math.min(45, Math.trunc(horizonDaysRaw))) : 30;
  ...
  const startDate = fromDate || calendarDateInTimeZone(tz);
  const dates = [];
  ...
  return NextResponse.json({ dates, fromDate: startDate, horizonDays });
}
